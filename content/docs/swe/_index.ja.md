...
![ユーザーのニーズ、ビジネス目標、制約とリスク、変化と不確実性という現実世界の文脈が、反復的なソフトウェアエンジニアリングへ流れ、横断的関心事が活動全体に適用され、利用可能なソフトウェアシステムの「運用から得た証拠」がエンジニアリングへ戻る関係](software-engineering-landscape.ja.webp "現実世界の文脈から利用可能なソフトウェアシステムまで")

ソフトウェアエンジニアリングは、ユーザーのニーズ、ビジネス目標、制約とリスク、変化と不確実性を含む、不完全で変化し続ける現実世界の文脈に応えます。構想・要求整理、設計、構築、テスト、デリバリー、運用と改善は認識しやすいライフサイクルを形成しますが、一方向の工程ではなく、活動間をフィードバックが結びます。セキュリティ、信頼性、保守性、性能は、全体の意思決定に影響する横断的関心事です。ソフトウェアが利用可能になると、運用から得た証拠がエンジニアリングへ戻り、文脈に対する理解を更新しながら、継続的な改善と適応を支えます。
