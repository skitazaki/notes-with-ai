...
| タスク（Task） | メッセージへの応答としてサーバーが作成・管理する、状態を持つ作業を表す |
| Part | メッセージまたは成果物に含まれるテキスト、ファイル参照、構造化データを表す |
| 成果物（Artifact） | 文書や構造化レコードなど、タスクが生成した結果を運ぶ |

クライアントは、まず適切なリモートエージェントを特定し、公開されたインターフェースを読み取ります。次に、新しいメッセージを送って作業を依頼します。このメッセージがやり取りの起点になります。

サーバーは、要求への応答をどのように表すかを判断します。単純なやり取りであれば、`Message` を直接返せます。状態管理、進捗の追跡、追加の入力、非同期の完了が必要な場合は、サーバーが `Task` を作成し、一意な `taskId` を生成してクライアントへ返します。その後、タスクは時間とともに進行し、追加の入力を要求し、状態更新を公開し、一つ以上の成果物を生成できます。同じやり取りを継続するとき、クライアントは既存の `taskId` を参照する新たなメッセージを送ります。
...
エージェントの作業は、通常の要求と応答より長く続く場合があります。クライアントはメッセージを送って作業を開始します。サーバーが状態を持つライフサイクルが必要だと判断した場合は、タスクを作成して返します。状態管理が不要なら、タスクを作成せずにメッセージを直接返すこともできます。作成されたタスクは実行を開始し、追加の入力や認可を待つために一時停止し、最終的に完了、失敗、拒否、キャンセルのいずれかに至ります。クライアントは遅延した応答をすべてネットワーク障害として扱うのではなく、これらの状態を理解する必要があります。

ここでは、業務上の依頼とプロトコル上の状態を区別することが重要です。クライアントは作業を依頼・委任し、サーバーはその結果となる `Task` の表現とライフサイクルを管理します。クライアントは、追加情報を渡したり同じタスクを継続したりするときに、サーバーが生成した `taskId` を後続のメッセージから参照できます。
