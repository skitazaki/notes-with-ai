...
![学習と推論を支える、計算、ストレージ、ネットワーク、サービング、検索、オーケストレーション、オブザーバビリティの AI インフラストラクチャ層。](ai-infrastructure.ja.webp)
...
ベクトルデータベースは、埋め込みを本番規模で保存、索引、絞り込み、管理、提供するためのインフラストラクチャの選択肢です。 [ベクトル検索](../context-engineering/vector-search/) を支えますが、両者は同義ではありません。ベクトル検索は検索技術であり、ベクトルデータベースはデータと索引を運用します。汎用データベースや検索エンジンもベクトル検索機能を提供でき、RAG に専用のベクトルデータベースが必要とは限りません。

[AI ゲートウェイ](ai-gateway/) では、モデルとプロバイダーのルーティングから、MCP ツール接続、エージェント間通信、ガバナンス、AI 固有のオブザーバビリティまで、この境界を詳しく説明します。
...
またオブザーバビリティ、回復性、スケーリング、インシデント対応、ライフサイクルガバナンスにも直結します。学習成果物、デプロイしたモデル版、埋め込み、キャッシュ、ルーティングルールを可視化できなければ、プラットフォーム全体を信頼できる状態に保てません。
