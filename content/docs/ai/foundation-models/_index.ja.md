...
言語とコードのモデルは、説明、抽出、下書き、質問応答、構造化変換を通じて多くの業務フローに適用できるため、特に注目されています。視覚、音声、マルチモーダルのモデルも、知覚が中心となる領域では同様に重要です。埋め込みモデルは直接の生成よりも表現、検索、順位付けを支え、テキスト、画像、コードなどの情報を、有用な意味的・構造的関係を捉える学習済みの数値表現へ変換します。表現モデル、比較方法、用途、ライフサイクルについては [埋め込み](embeddings/) を参照してください。
...
その結果、アプリケーションアーキテクチャは、ゲートウェイ、オーケストレーション、検索システム、安全制御、キャッシュ、オブザーバビリティ、ライフサイクルガバナンスへ重心を移します。
...
{{< cards >}}
{{< card link="embeddings/" title="埋め込み" icon="document-text" subtitle="テキスト、画像、コードなどの情報を表す学習済みの数値表現" >}}
{{< /cards >}}
