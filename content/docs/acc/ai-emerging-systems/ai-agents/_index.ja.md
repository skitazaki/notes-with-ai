...
![利用者が境界付きでタスクを AI エージェントへ委譲し、エージェントの要求をポリシー適用ポイントとポリシー判断ポイントが評価する認可フロー。実行 Principal、委譲元 Subject、タスク範囲、リソース、操作、承認、有効期限、ツール範囲を用いて許可または拒否を判断し、その結果を監査ログへ記録する。](ai-agents-autonomous-authorization.ja.webp "AI エージェントと自律的認可")
