...
このフローでは、利用者が境界付きでタスクを委譲し、エージェントの要求を Policy Enforcement Point（PEP）が受け取り、Policy Decision Point（PDP）が Principal と Subject と委譲制約を評価し、許可・拒否の結果を監査ログに残します。
...
PEP は、ツール呼び出し、API 要求、ワークフローステップを受け止めて、認可要求へ変換する部品です。PDP は、その要求をポリシー、委譲文脈、承認状態、リソース状態に照らして評価し、許可・拒否、場合によっては追加義務付きの結果を返す部品です。
