...
このセクションは、相補的な 6 つの観点をまとめたハブです。まず以下の比較で全体像を捉え、その後、各トピックページでそれぞれのレンズを詳しく確認できます。
...
現代のシステムは、1 つの視点だけでは理解しきれません。同じプラットフォームでも、依存関係を考えるための構造ビュー、実行時の振る舞いを見る運用ビュー、設計上の優先順位を示す戦略ビュー、チーム責任を明らかにする責任分担ビュー、関係者に伝えるための表現ビューが必要になることがあります。

これらは互いに競合するアーキテクチャ定義ではなく、相補的なレンズです。どれか 1 つだけが「アーキテクチャ」なのではありません。各ビューは、特定の目的のために投影されたアーキテクチャの一面です。
...
[レイヤー](layers/)は、構造上の抽象化を表します。何が何に依存しているか、どの抽象化がどの抽象化の上に成り立つか、依存関係はどの方向へ流れるべきか、どの部分を変更から隔離すべきかを考える助けになります。

レイヤーは、アプリケーションアーキテクチャ、プラットフォームの積み上がり、プロトコルモデル、実行基盤の抽象化を考えるときに有用です。一方で、図の中に並ぶ横長の箱を何でもレイヤーと呼んだり、デプロイ構成と混同したりすると誤解を招きます。
...
[プレーン](planes/)は、運用上の責任や制御経路を表します。誰が実行を制御しているか、誰がトラフィックやデータを処理しているか、ポリシーやテレメトリをどの経路で扱うか、どの関心事が構造上の境界を横断するかを明らかにします。

コントロールプレーン、データプレーン、オブザーバビリティプレーン、ポリシープレーン、ワークフロープレーンは、しばしば複数のレイヤーをまたぎます。そのため、構造としてはレイヤー化されたシステムが、同時にプレーンによって運用されることがあります。

#### フローとパイプライン

[フローとパイプライン](flows-and-pipelines/)は、時間の中での移動を表します。リクエスト、イベント、データ、ジョブ、承認、フィードバックループが、どのようにシステム境界を通過していくかを示します。

フロー指向のビューは、順序、変換、障害点、キュー、リトライ、引き継ぎを可視化します。実行中に何が起こるかを示すことで、構造ビューを補完します。
...
[ピラー](pillars/)は、信頼性、セキュリティ、拡張性、コスト効率、保守性、運用容易性、開発者体験などのアーキテクチャ上の優先事項を表します。

ピラーは実行時コンポーネントではなく、意思決定のためのレンズです。特定の制約の下で、なぜある設計が別の設計より望ましいのかを説明する助けになります。
...
[責任境界](ownership-boundaries/)は、変更と運用に関する責任の切れ目を表します。技術的な境界と関係はありますが、サービス、レイヤー、デプロイ単位、組織図と同じものではありません。
...
[ビュー](views-and-viewpoints/)は、対象読者と目的に応じて、関心事、詳細度、表現方法を選び取る伝達用の成果物です。ビューポイントは、そのビューを構成するための見方や枠組みを定義します。

開発者向けビュー、プラットフォーム運用ビュー、セキュリティレビュー向けビュー、経営層向けビューは、同じシステムを扱いながらも強調する情報が異なります。レイヤー図は依存方向を、フロー図はリクエストの移動を、責任分担マップは変更と運用の責任を説明できます。
