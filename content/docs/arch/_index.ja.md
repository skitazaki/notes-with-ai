...
[フローとパイプライン](dimensions/flows-and-pipelines/) を使い、構成要素の間を通るリクエスト、イベント、データ、障害経路を追います。 [アーキテクチャ原則](principles/) で組織の持続的な指針を、 [アーキテクチャ制約](constraints/) で選択肢が満たすべき境界を定めてから、 [意思決定フレームワーク](decision-frameworks/) へ進みます。特定の読者に結果を説明するときは、 [ビューとビューポイント](dimensions/views-and-viewpoints/) に立ち返ります。
...
ピラーは何が重要かを示し、原則はその優先事項を持続的な指針へ変換します。制約は、検討できる選択肢の範囲を境界づけます。
意思決定フレームワークは、その両方を個別の選択へ適用します。
レイヤー、プレーン、フローは結果として得られるアーキテクチャを記述し、ビューは特定の読者に関係する部分を提示します。

![関心事からアーキテクチャへ: 関心事からピラーと制約へ進み、ピラーから原則へ、原則と制約から意思決定フレームワークへ進む。アーキテクチャ上の判断は、レイヤー、プレーン、フローとパイプライン、ビューとビューポイントで表現する](from-concern-to-architecture.ja.webp "関心事からアーキテクチャへ")
