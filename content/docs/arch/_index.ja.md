...
システムの目的、周囲の環境、関係する人や外部システムから始めます。 [アーキテクチャの観点](dimensions/) で考えるためのレンズを選び、 [責任境界](dimensions/ownership-boundaries/) で責任の所在を明らかにし、 [ビューとビューポイント](dimensions/views-and-viewpoints/) で対象読者に合った説明の枠組みを定めます。

### システムを構成する主要な要素は何か

システムの内側へ進み、構造上・運用上の大きな構成要素を捉えます。 [レイヤー](dimensions/layers/) では抽象化と依存方向を、 [プレーン](dimensions/planes/) では運用上の責務の分離を扱います。 [ピラー](dimensions/pillars/) は、それらの選択を形づくる品質特性や制約を明らかにします。

### 主要な構成要素はどのように連携するか

[フローとパイプライン](dimensions/flows-and-pipelines/) を使い、構成要素の間を通るリクエスト、イベント、データ、障害経路を追います。 [アーキテクチャ原則](principles/) を使って組織の持続的な指針を [意思決定フレームワーク](decision-frameworks/) へ取り込み、その連携を関心事、トレードオフ、アーキテクチャ上の判断へ結びつけます。特定の読者に結果を説明するときは、 [ビューとビューポイント](dimensions/views-and-viewpoints/) に立ち返ります。
...
| **システム**               | [アーキテクチャの観点](dimensions/)、[ピラー](dimensions/pillars/)、[アーキテクチャ原則](principles/)、[意思決定フレームワーク](decision-frameworks/) |
...
アーキテクチャには、密接につながる 2 つの役割があります。
チームがシステムについて考えることを助け、その考え方を、理解、検証、承認、構築、運用する必要がある人へ伝えることを助けます。

共有されない判断は個人の中にとどまり、レビューが困難になります。
判断を伴わない伝達は、見た目が整っていても妥当な意思決定を反映していない成果物を生みます。
有用なアーキテクチャは、この 2 つの間を繰り返し行き来します。
...
判断と共有は、関心事を通じてつながります。
関心事は、チームが何を理解する必要があるか、どの観点とトレードオフを検討するべきか、どのビューを使って想定読者へ結果を伝えるかを決めます。
...
3. 選択肢とトレードオフを検討する。
4. 想定読者向けのビューを作る。
5. ビューから得たフィードバックを使って考え方を改善するか、意思決定を支える。

![関心事からビューへ: 関心事の特定から、観点とトレードオフについて考え、対象読者向けのビューを作り、ビューから得たフィードバックで考え方を改善するか意思決定を支えるまでの 5 ステップ](from-concerns-to-views.ja.webp)
...
実務では、この判断と共有のループに、このセクションの概念を個別の用語ではなく、つながった体系として適用します。
ピラーは何が重要かを示し、原則はその優先事項を持続的な指針へ変換します。
意思決定フレームワークは、その指針を個別の選択へ適用します。
レイヤー、プレーン、フローは結果として得られるアーキテクチャを記述し、ビューは特定の読者に関係する部分を提示します。

![関心事からアーキテクチャへ: ピラーからアーキテクチャ原則、意思決定フレームワーク、アーキテクチャ上の判断へ進み、レイヤー、プレーン、フローとパイプライン、ビューとビューポイントで表現する流れ](from-concern-to-architecture.ja.webp "関心事からアーキテクチャへ")

このマップは実務的な進行を示しますが、必須の一方向プロセスではありません。
判断から新しい関心事が見つかることも、ビューによって原則の適用しにくさが分かることも、構造や実行時の分析によって根拠が変わることもあります。
以下のトピックページでは、この体系の各部分と、それぞれが答える問いを説明します。
