...
[フローとパイプライン](flows-and-pipelines/) を使い、構成要素の間を通るリクエスト、イベント、データ、障害経路を追います。 [アーキテクチャ原則](principles/) を使って組織の持続的な指針を [意思決定フレームワーク](decision-frameworks/) へ取り込み、その連携を関心事、トレードオフ、アーキテクチャ上の判断へ結びつけます。特定の読者に結果を説明するときは、 [ビューとビューポイント](views-and-viewpoints/) に立ち返ります。
