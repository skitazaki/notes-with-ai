...
[Architecture Principles](../principles/) provide durable guidance across many decisions, while [Architecture Constraints](../constraints/) define the boundaries an option must satisfy. A decision framework applies both while reasoning through one particular choice. A principle can establish a default or burden of proof, but the framework still tests viable options against context, evidence, and consequences.
