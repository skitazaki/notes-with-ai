...
[アーキテクチャ原則](../principles/) は多くの意思決定を通じて持続する指針を与え、 [アーキテクチャ制約](../constraints/) は選択肢が満たすべき境界を定めます。意思決定フレームワークは、その両方を個別の選択へ適用します。原則は既定の方向や説明責任の所在を定められますが、フレームワークは引き続き、実行可能な選択肢を文脈、根拠、結果に照らして検証します。
