...
ガバナンスは、正当な確信を持ってデータを利用できるようにするために存在します。組織やシステムの境界を越えて、権限、義務、例外を明確にします。有効なガバナンスは、信頼できる分散型のデータ利用を可能にするものであり、すべての判断を中央承認に集約するものではありません。
...
flowchart TB
  ...
  EM --> RC["レビューと変更"]
  RC --> SP
  A["オーナーシップとスチュワードシップ"] --- SP
  A --- DR
  A --- EM
  M["メタデータ：資産、分類、リネージ、管理策、証跡"] --- PS
  M --- C
  M --- EM
```
