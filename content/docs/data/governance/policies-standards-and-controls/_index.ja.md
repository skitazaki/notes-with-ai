...
![原則がポリシー、標準、管理策をまとめたガバナンス機構全体へ方向性を与え、証跡とフィードバックが保証を支える関係](policies-standards-controls.ja.webp "ポリシー、標準、管理策")
...
これらの要素は追跡可能な関係を形成しますが、すべてが同じ種類の階層ではなく、必ず一対一に対応するものでもありません。原則はガバナンスの仕組み全体へ方向性を与え、詳細なルールがない場合の判断を導きます。ポリシーは定義された範囲で成果を必須にします。標準は検証可能な共通要件を示し、不要なばらつきを減らします。管理策はその要件に対して動作する行為または機構であり、別の文書ではありません。証跡はレビュー、フィードバック、保証を支えるために保持される結果です。
