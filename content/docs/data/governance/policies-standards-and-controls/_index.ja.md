...
この階層は、実装できない抽象的な方針と、目的が不明な技術管理策という二つの問題を防ぎます。各層は上位の層へ追跡可能でありながら、それぞれの対象読者に適した内容である必要があります。経営層とポリシーオーナーが必須成果を示し、ドメインと技術の専門家が共通要件へ変換し、デリバリー・プラットフォームチームが管理策を運用します。
...
これらの用語は関係していますが、同じ意味ではありません。原則は詳細なルールがない場合の判断を導きます。ポリシーは定義された範囲で成果を必須にします。標準は検証可能な要件を示し、不要なばらつきを減らします。管理策は別の文書ではなく、動作または機構です。証跡はレビューと保証を支えるために保持される結果です。
