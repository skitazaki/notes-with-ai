...
マスタデータは参照データと混同されがちですが、実務ではこの違いは重要です。 [参照データ](/docs/data/management/reference-data/) は、許容される値や共有コード体系を管理します。一方、マスタデータはエンティティそのものを表します。たとえば製品カテゴリコードは参照データですが、そのカテゴリに属する製品自体はマスタデータです。
