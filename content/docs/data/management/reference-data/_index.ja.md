...
参照データ管理は、共有語彙を確立します。すべてのシステムに同じ値を格納させることが目的ではありません。それぞれのローカル値を、合意された意味へ予測可能な形で対応付けることが重要です。
...
| 主な関心事   | 共有アイデンティティ     | 共有語彙                       |
| 一般的な操作 | レコードの照合と統合     | コードの定義、マッピング、配信 |

製品はマスタデータであり、そのライフサイクル上の状態やカテゴリコードは参照データです。両者は相互に関係しますが、解決する問題は異なります。 [マスタデータ管理](/ja/docs/data/management/mdm/) はエンティティの共有アイデンティティを確立し、参照データ管理はエンティティに付与する値の共有語彙を確立します。
...
ローカルまたはレガシーな複合コードでは、複数の値を1つのコンパクトな識別子へ連結する場合があります。たとえば `JPN-YEN-LIVE` を共有語彙へマッピングするには、ソース側の3つの要素を分割して検証したうえで、国の `JPN` を `JP`、通貨の `YEN` を `JPY`、状態の `LIVE` を `ACTIVE` へ変換するルールが必要です。このマッピングには単純なコードの読み替えだけでなく、解析と正規化が伴うため、要素の順序、区切り文字、欠損値の扱い、変換規則を明示的に統制する必要があります。
...
有効期間の管理は、値が業務上いつ有効であるかを記録します。その有効時刻と、システムが値を記録または認識した時刻を区別するには、システム時刻やトランザクション時刻を別に保持する必要があります。バージョンを保持すれば、監査や過去データの処理に使用したコードセット全体を再現できます。廃止したコードも、新規処理で使用できなくなった後に、過去のレコードから引き続き解決できる必要があります。
