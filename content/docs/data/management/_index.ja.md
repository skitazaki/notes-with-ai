...
title: "データマネジメント"
weight: 4
prev: "/ja/docs/data/sharing"
next: "/ja/docs/data/management/data-quality-dimensions"
---

データマネジメント（Data Management）は、データが時間の経過とともに信頼でき、利用可能で、持続可能な状態を維持するための体系的な取り組みです。本書は、価値創出層を支える活用基盤層の一部として位置づけられるデータマネジメントの概念を整理するものです。分析や共有によって成果を生み出すには、その前提としてデータの正確性、整合性、可用性が確保されている必要があります。データマネジメントは、その前提条件を安定的に維持する役割を担います。

[データガバナンス](/ja/docs/data/governance/) は期待事項、意思決定権、説明責任を定め、データマネジメントはそれらを満たすための品質、アクセス性、ライフサイクル、サービスに関する継続的な業務を実行します。

本書の対象読者は、技術責任者、アーキテクト、シニアエンジニアなど、全体像の理解を必要とする読者です。特定の製品や実装方法ではなく、概念と目的に焦点を当てます。

## データ品質管理

データ品質管理（Data Quality Management）は、データが対象とする実世界のエンティティや事象を正しく、一貫して表現できているかを維持するための枠組みです。

データ品質は多面的ですが、多くの実務フレームワークは次の6つを中核的な品質観点として共有しています。

- **正確性（Accuracy）**: 記録された値が実態を正しく表していること
- **完全性（Completeness）**: 必要な属性や値が欠けていないこと
- **一貫性（Consistency）**: システムやデータセット間で矛盾がないこと
- **適時性（Timeliness）**: 利用目的に対して十分に新しい状態であること
- **妥当性（Validity）**: 定義された形式、ルール、制約に適合していること
- **一意性（Uniqueness）**: 意図しない重複レコードや重複したエンティティ表現が存在しないこと

これら6つの品質観点は、多くのデータアーキテクチャに共通して適用でき、測定可能な期待値に落とし込みやすいため、実務上の最小共通コアとみなせます。一方で、整合性制約、信頼性、関連性、実行時の観測可能性なども重要ですが、最小の共通コアというよりは、隣接する品質特性または拡張的な観点として扱う方が適切です。

主な構成要素には、検証ルール、プロファイリング、異常検知、継続的品質モニタリングが含まれます。
...
例えば、受注データでは、識別子が欠損していないこと、通貨コードが妥当であること、取引日が未来日ではないこと、さらに同一取引が重複して表現されていないことが求められます。これらのチェックだけで誤りを完全になくせるわけではありませんが、データの状態を観測可能かつ測定可能にします。

品質が可視化されることで、利用者はそのデータが特定の目的に適しているかを判断しやすくなります。これにより、誤りが知らないうちに下流へ伝播することを防ぎ、分析や業務処理の信頼性を高めることができます。
...
マスタデータ管理（Master Data Management）は、組織横断で共有される中核エンティティを一貫した形で維持する仕組みです。顧客、製品、組織、拠点など、複数のシステムや業務領域で共通利用される概念が対象です。
...
例えば、同一顧客が複数システムで別々に登録されている場合、売上集計や与信管理は正確性を欠きます。マスタデータ管理は、組織全体で共有される事実の基盤を確立する役割を果たします。
...
詳細は [データライフサイクル管理](/ja/docs/data/management/lifecycle-management/) を参照し、生成、活用、保持、アーカイブ、計画的な廃棄、そして各状態に必要な証跡と責任の整理を扱う運用モデルを確認してください。
...
データマネジメントは、データを単なる記録から、継続的に活用できる基盤へと昇華させるための体系です。活用基盤層における中核的な要素として、信頼性と運用効率を支える役割を果たします。
