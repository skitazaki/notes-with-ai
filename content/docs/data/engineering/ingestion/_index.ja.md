...
データ取り込み（Data Ingestion）は、生成または収集されたデータをデータプラットフォームの境界を越えて確実に転送し、下流の処理や利用に向けた永続的な管理対象状態へ移すエンジニアリングプロセスです。対象は運用データベース、ファイル、API、アプリケーション、デバイス、パートナー、公開データなどです。単なるバイト転送ではなく、データ提供側への影響を抑えながら、意味と配信保証を維持することが課題です。
...
**イベント・メッセージ取り込み** は、データ提供側がブローカーやエンドポイントへ発行したレコードを受信します。確認応答、保持、順序、バックプレッシャーを明確にします。
...
**プル** ではプラットフォームがソースへ要求し、実行頻度と再試行を制御します。**プッシュ** ではデータ提供側または中継システムがデータを送信し、低遅延にしやすい一方、受信側は急増を吸収する必要があります。
...
ランディングゾーンは、取り込みが受領データを最初に確定する、永続的なプラットフォーム管理下の場所または状態です。ソース境界の近くにありますが、ソースや収集手段ではありません。ランディングへの到達によって、データ提供側や転送処理が管理するデータから、プラットフォームが保持、識別、運用できるデータへ移ったことを示します。

ランディングでは一般に受領時の表現を保持し、後続処理が失敗または変更された場合にも、ソースから再取得せず実行できるようにします。取り込み方式によって、その永続的な状態は不変なファイルやオブジェクト、追記専用ログ、ソースに近い形のテーブルなどになります。`landing` や `raw` という名前の物理ディレクトリである必要はありません。

ランディングの設計では、次の責任を明確にします。

- **永続性とリプレイ** — 後続処理の復旧や再現に必要な期間、受領済みデータを保持し、保持期間と削除規則を定める
- **識別と来歴** — トレーサビリティと重複排除に必要なソース、到着時刻、取り込み実行または位置、スキーマの文脈などを記録する
- **検証と隔離** — ランディングを業務変換の場に変えず、転送、形式、基本的な整合性を検証し、安全に受け入れられないデータを隔離する
- **分離とアクセス** — 特に機密項目を含む場合や後続の品質管理を通過していない場合に、ソースに近いデータの直接利用を制限する
- **完了条件** — 復旧可能になる前に確認応答やチェックポイントを進めないよう、永続的な受け入れが完了したとみなす条件を定める

ランディング、ステージング、raw、bronze は同じ意味で使われることもありますが、普遍的な定義はありません。ゾーン名だけに依存せず、それぞれの責任、変更可否、保持期間、許可する変換を定義する必要があります。

## 取り込みと処理の境界

取り込みは、技術的なソースインターフェースから読み取り、データを確実に転送し、配信位置と来歴を特定できる形で永続化する責任を主に担います。 [データ処理](../processing/) は、フィルタリング、結合、集計、付加情報、業務ルールにより利用目的に合う表現へ変換します。実際の基盤では軽い正規化や検証を取り込みに含めることもありますが、責任の境界を可視化すると障害、リプレイ、所有を判断しやすくなります。
