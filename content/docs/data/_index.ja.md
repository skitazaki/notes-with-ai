...
{{< card link="collection/" title="データ収集" icon="collection" subtitle="目的、来歴、制約を明確にしてソース側でデータを選択・記録する" >}}
...
### [データ収集](/ja/docs/data/collection/)

データ収集の中心となる問いは、**どのデータを、どこから、どのような条件のもとで取得し、ソース側でどのように記録するか**です。

- **ソース選定** – 運用システム、アプリケーション、人、デバイス、パートナー、公開ソース、商用提供元の正規性と目的適合性を評価
- **収集設計** – 目的、母集団、観測単位、範囲、粒度、頻度、タイミング、標本抽出を定義
- **記録方法** – 計測、取引記録、測定、テレメトリ、調査、フォーム、外部データ取得
- **取得時の文脈** – 来歴、ソース所有者、信頼性、限界、許可の根拠、利用制限

データ収集は、エンジニアリングが始まる前に、組織が何を観測可能にするかを決めます。データが生成または収集された後は、 [データ取り込み](/ja/docs/data/engineering/ingestion/) がプラットフォーム境界を越えて、永続的な管理対象状態へ確実に移します。CDC、増分読み取り、再試行、チェックポイント、リプレイ、配信保証は、データ収集ではなく取り込みの仕組みです。
