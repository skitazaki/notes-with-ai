...
## データ理解を支える仕組み、自動化、エコシステム

データ理解は、手作業による調査だけで成り立つものではありません。メタデータはコンテキストを提供し、自動化は根拠の収集を反復可能にし、生成 AI は解釈を支援し、より広いツールのエコシステムはこれらの機能を結び付けます。いずれも調査を加速できますが、観測データやドメイン知識に基づく検証に代わるものではありません。
...
データカタログはこれらのシグナルを検索可能にし、共有された [メタデータ](../../metadata/) レイヤーはそれらを結び付けます。ただし、メタデータも疑う余地のない真実ではなく、根拠の一つです。説明が古い、リネージが手作業を含んでいない、宣言された制約が強制されていない、自動推論されたラベルが誤っている可能性があります。十分な理解を得るには、メタデータを観測データおよびドメインの知見と比較します。
