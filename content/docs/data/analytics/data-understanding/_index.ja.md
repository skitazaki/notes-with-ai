...
CRISP-DM は、データマイニングプロジェクトで広く使われている反復型のプロセスモデルであり、データ理解はそのフェーズの一つです。ただし、この用語の範囲はコミュニティによって異なります。このページでは、分析、データエンジニアリング、機械学習、生成 AI に先立って行う、より広い活動として整理します。ここで示す分類は編集上の整理であり、普遍的な業界標準ではありません。
