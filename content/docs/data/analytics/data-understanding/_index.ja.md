...
この用語は CRISP-DM などのデータマイニングやデータサイエンスの方法論でも使われますが、その範囲はコミュニティによって異なります。このページでは、分析、データエンジニアリング、機械学習、生成 AI に先立って行う、より広い活動として整理します。ここで示す分類は編集上の整理であり、普遍的な業界標準ではありません。

![データの発見、構造と意味の理解、プロファイリング、関係性の理解、来歴とコンテキスト、品質評価、適合性評価を、まとまりのある活動群として配置したデータ理解の全容図](data-understanding-activities.ja.webp "データ理解")

この図は、データ理解を複数のテクニックや活動を包含する概念として示しています。各活動は異なる根拠を提供し、固定された順序ではなく反復的に実施される場合があります。
...
統計が説明するのは振る舞いであって、意味ではありません。強い相関は、有用な因果関係ではなく、上流で共有された計算、母集団フィルター、データリークを反映している可能性があります。外れ値はエラーかもしれませんが、まれに発生する有効な値や、データセットで最も重要なレコードかもしれません。
...
一見した異常が、パイプラインのスケジュール、遅延到着イベント、スキーマ移行、上流での訂正によるものなら、運用コンテキストが不可欠です。リネージと運用メタデータについては、 [メタデータ](../../metadata/) で詳しく説明しています。
...
- タイムスタンプのメタデータは UTC を示していますが、夏時間の切り替わり付近のサンプルとソースオーナーへの確認によって、上流で変換されたかを確定します。
...
- パイプラインのメタデータから鮮度を確認し、テストアカウントを除外する上流変換を特定します。
...
- [IBM SPSS Modeler: Data Understanding Overview](https://www.ibm.com/docs/en/spss-modeler/19.0.0?topic=understanding-data-overview) — CRISP-DM におけるデータ理解フェーズと、探索、品質、文書化との関係
